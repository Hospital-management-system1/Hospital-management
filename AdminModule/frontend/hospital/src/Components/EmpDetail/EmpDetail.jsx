...
import AddEmpDatail from "./AddEmpDetail";
...
const EmpDetail = () => {
  ...
  function getData() {
    fetch("http://localhost:5999/getEmpDetail").then((result) => {
  ...
  function deleteEmpDetail(id) {
    fetch(`http://localhost:5999/deleteEmpDetail/${id}`, {
      method: "DELETE",
      headers: {
    ...
    <div>
      <div className="overflow-x-auto">
        <table className="table">
          ...
          <thead>
            <tr>
              <th className="text-center text-base">Profile ID</th>
              ...
              return (
                <tr key={index}>
                  ...
                  <td className="text-center">
                    {currEle.dob.toString().slice(0, 10)}
                  </td>
                  <td className="text-center">
                    <div>
                      <button className="mr-2 ">
                        <EditEmpDetail emp_id={currEle.profile_id} />
                      </button>
                      <button
                        className=" btn btn-outline btn-error btn-sm text-white"
                        onClick={() => deleteEmpDetail(currEle.profile_id)}
                      >
                        <svg
